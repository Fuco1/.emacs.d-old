...
(defcustom ahk-bin nil
  "The binary to run AHK script."
  :type 'string
  :group 'ahk-mode)
...
(defvar ahk-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c C-r") 'ahk-reload-file)
...
(defun ahk-reload-file ()
  "Reload this file.  If not saved, save first"
  (interactive)
  (save-buffer)
  (let ((cmd (concat ahk-bin " reload.ahk \"" (buffer-file-name) "\"")))
    (shell-command cmd)))
...
\\{ahk-mode-map}"
  ...
  (setq indent-region-function 'ahk-indent-region)
  (make-local-variable 'indent-line-function)
  (setq indent-line-function 'ahk-indent-line)
